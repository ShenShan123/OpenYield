...
class NSGA2Utils:
    ...
    def __crossover(self, individual1, individual2):
        ...
        for i in genes_indexes:
            ...
            # Round to integer if the variable type is integer
            if self.problem.variables_type[i] == 'int':
                value1 = int(round(value1))
                value2 = int(round(value2))
                
            child1.features[i] = value1
            child2.features[i] = value2
    ...
    def __mutate(self, child):
        ...
        for gene in range(num_of_features):
            ...
            if u < 0.5:
                delta_value = delta * (child.features[gene] - self.problem.variables_range[gene][0])
            else:
                delta_value = delta * (self.problem.variables_range[gene][1] - child.features[gene])
            
            # Round delta for integer variables
            if self.problem.variables_type[gene] == 'int':
                delta_value = int(round(delta_value))
                if delta_value == 0:
                    delta_value = 1 if random.random() < 0.5 else -1
            
            child.features[gene] += delta_value
            ...
            # For integer variables, ensure the value is integer
            if self.problem.variables_type[gene] == 'int':
