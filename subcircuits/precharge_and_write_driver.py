...
class Precharge(SubCircuitFactory):
    """
    Precharge circuit for SRAM bitlines with dynamically adjusted strength.
    """
    NAME = "Precharge"
    NODES = ('VDD', 'BL', 'BLB', 'PRE')  # Power, BL, BLB, Precharge Enable
    ...
    def calculate_dynamic_width(self, base_width, num_rows):
        ...
        """
        scaling_factor = 1 + (num_rows - 1) * 0.05  # Example: 5% increase per additional row
...
class WriteDriver(SubCircuitFactory):
    """
    Write driver circuit for SRAM with dynamically adjusted strength.
    """
    NAME = "WriteDriver"
    NODES = ('DIN', 'BL', 'BLB', 'EN', 'VDD', 'VSS')  # Data In, BL, BLB, Enable, VDD, GND

    def __init__(self, nmos_model_name, pmos_model_name,
                 base_nmos_width, base_pmos_width, length, num_rows):
        super().__init__()
