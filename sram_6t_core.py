...
class SRAM_6T_Cell(SubCircuitFactory):
    ...
    def __init__(self, nmos_model_name, pmos_model_name,
                 nmos_width=0.1e-6, pmos_width=0.2e-6, 
                 access_width=0.15e-6, length=45e-9,
                 w_rc=False,
    ...
    def add_6T_cell(self, bl_node, blb_node, wl_node):
        ...
        # Access transistors
        self.M(1, bl_node,  wl_node, 'Q',  self.NODES[1], model=self.nmos_model_name, w=self.access_width, l=self.length)
        self.M(2, blb_node, wl_node, 'QB', self.NODES[1], model=self.nmos_model_name, w=self.access_width, l=self.length)
        print(f"[DEBUG] M1-M2: Access transistorsNMOS ({self.nmos_model_name}) W={self.access_width} L={self.length})")

        # Cross-coupled inverters
        self.M(3, 'Q', 'QB', self.NODES[1], self.NODES[1], model=self.nmos_model_name, w=self.nmos_width, l=self.length)
        self.M(4, 'Q', 'QB', self.NODES[0], self.NODES[0], model=self.pmos_model_name, w=self.pmos_width, l=self.length)
        self.M(5, 'QB', 'Q', self.NODES[1], self.NODES[1], model=self.nmos_model_name, w=self.nmos_width, l=self.length)
        self.M(6, 'QB', 'Q', self.NODES[0], self.NODES[0], model=self.pmos_model_name, w=self.pmos_width, l=self.length)

        # NOTE: Add small load cap to data nodes to balance the write time.
        self.C(f'Cg_Q', 'Q', self.gnd, 0.001 @ u_pF)
        self.C(f'Cg_QB', 'QB', self.gnd, 0.001 @ u_pF)

        print(f"[DEBUG] M3-M6: Cross-coupled inverters (NMOS={self.nmos_model_name} W={self.nmos_width} L={self.length}"+
              f"      PMOS={self.pmos_model_name} W={self.pmos_width} L={self.length})")  
...
class SRAM_6T_Array(SubCircuitFactory):
    ...
    def __init__(self, num_rows, num_cols, nmos_model_name, pmos_model_name, w_rc=False):
        ...
        subckt_6t_cell = SRAM_6T_Cell(nmos_model_name, pmos_model_name, w_rc=w_rc)
