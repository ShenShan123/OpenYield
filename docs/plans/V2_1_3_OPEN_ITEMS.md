...
| Found during V2.1.9: the write -> write slot (`TWSLOT`) exceeds the qualification scorer's `restore_budget` (0.8 x class budget) at the classes up to 64 rows at SS | Open (decision: relax the scorer check for the write slot or grow the small classes) | [write-hold record](../design/WRITE_HOLD_V2_1_9.md), section 8 |
