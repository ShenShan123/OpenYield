...
| Found during the V2.1.8 review: the read wordline outlived the sense enable by 1.1 to 2.2 ns; the boundary orders of the precharge, write slot and enables were set by path length only; a deselect dropped the write drivers with the wordline at half VDD | Fixed in V2.1.8 (sense-timed read wordline, enable observers, selected slot) | [enable-overlap record](../design/ENABLE_OVERLAP_V2_1_8.md) |
| Found during the V2.1.9 audit: the write drivers released with the wordline enable (and the held request at write -> read) while the local wordline was still on, up to 0.90 V at 512x4 SS | Fixed in V2.1.9 (busy-wordline hold, slot-arm latch, runtime check) | [write-hold record](../design/WRITE_HOLD_V2_1_9.md) |
| Found during the V2.1.9 worst-PVT sweep: under local mismatch at SS 0.9 V / 125 C the 256x4 6T and 512x4 6T-mux reads delivered the data inside the checker's 0.02 T guard; the slot-arm latch made mux read decks lose their DC operating point | Fixed in V2.1.9 (read classes of 128 to 512 rows raised by a mismatch rule; latch seeded with `.IC`) | [write-hold record](../design/WRITE_HOLD_V2_1_9.md), sections 5 and 6 |
| Found during V2.1.9: the write -> write slot (`TWSLOT`) exceeds the qualification scorer's `restore_budget` at the classes up to 64 rows at SS | Closed in V2.1.10 (the budget was the measured read phase, not the class; the slot also missed the runtime restore check at 32x16 / 32x32: the write-data latches now hold on the registered write, the drivers stay on between writes, and the scorer scores the slot against the clock-high phase) | [write-latch record](../design/WRITE_LATCH_V2_1_10.md) |
