...
| `wl_en` / `WL{row}` | `wl_pdrive(access_clk_bar)`, wordline driver | whole clock-low phase, `T/2`, starting `TCLK_WLEN` = 112-190 ps after the edge (287-290 ps for 512-column write decks: clock-buffer load) | edge time grows with rows / cols (buffer taper) | `T/2 >= low_wc * (1 + m)` |
| `w_en` (V2.1.6) | `AND3(we_hold, cs, wl_en \| (pre_ready & pre_off_ready))` | the write slot of the clock-high phase (from the previous wordline and the physical precharge observed off) through the access | bitline drive grows with rows (driver class) | `T/2 >= TWSLOT_wc * (1 + m)` |
| `PRE` | `NAND3(clk_buf, cs_pre, pre_ready & !we_hold)` + `PRE_BUF` | whole clock-high phase of a read cycle, `T/2`, after the replica wordline is observed off; inhibited in a write cycle (V2.1.6) | restore time 190-430 ps (rows, cols) | `T/2 >= high_wc * (1 + m)` |
