...
class MainEntranceTests(unittest.TestCase):
    ...
    def test_interconnect_setting_selects_distributed_wires_in_memory(self):
        ...
        # default and explicitly selected wiring must both be distributed.
        self.assertIsNone(main_sram.INTERCONNECT_CONFIG)
        before = yaml_digests()
        with tempfile.TemporaryDirectory() as temp, redirect_stdout(io.StringIO()):
            default = main_sram.configure(2, 2, False, 'TT', main_sram.CELL_6T)
            default_deck = str(main_sram.build_testbench(default, temp, variation_mode='nominal')
        ...
        self.assertIn('Rwire_WL', default_deck)
