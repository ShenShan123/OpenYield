...
class TimingLookupTests(unittest.TestCase):
    ...
    def test_6t_budgets_keep_250ps_of_read_output_margin_at_every_class_bound(self):
        ...
        for rows, cols, plain_ns, mux_ns in cases:
            with self.subTest(rows=rows, cols=cols):
                ...
                self.assertEqual(muxed.table_version, 'v2.1.7-timing-6')
        ...
        for rows, cols, ns in cases:
            for mux in (True, False):
                with self.subTest(rows=rows, cols=cols, mux=mux):
                    ...
                    self.assertEqual(timing.table_version, 'v2.1.7-timing-6')
    ...
    def test_custom_table_paths_margin_and_manual_override(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            try:
                os.chdir(temp)
                self.assertEqual(load_timing_lookup('sram_compiler/sizing/timing_lookup.json')['version'], 'V2.1.7')
            finally:
                os.chdir(previous)
    ...
    def test_cli_period_metadata_and_repeated_runs_preserve_original_evidence(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(summary['compiler_version'], 'V2.1.7')
