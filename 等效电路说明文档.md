...
mc_testbench = Sram6TCoreMcTestbench(
    ...
    use_equivalent=True,    # 开启等效电路
    ...
)
```

`use_equivalent=True` 时必须同时设置 `w_rc=True`，否则缺少线阻会低估大阵列延迟。
...
| 参数 | 默认值 | 说明 |
|------|-------|------|
| `use_equivalent` | `False` | `True` = 等效电路；`False` = 完整电路（参考基准） |
...
```python
# 完整电路
mc_testbench = Sram6TCoreMcTestbench(..., use_equivalent=False, w_rc=True, pi_res=100@u_Ohm, pi_cap=0.001@u_pF)

# 等效电路（调整 pi_res / pi_cap 使延迟接近完整电路）
mc_testbench = Sram6TCoreMcTestbench(..., use_equivalent=True,  w_rc=True, pi_res=100@u_Ohm, pi_cap=0.001@u_pF)
```
...
```
Sram6TCoreMcTestbench(use_equivalent=True)
  └─ Sram6TCore.build_array()
       ...
       └─ add_equivalent_circuit()  ← 由 sram_cell_add_equivalent.py 注入
            ...
            ├─ SRAMCellParasiticTester.get_static_power_r()
            ├─ 添加各非目标行的 WL RC（PI 型）
...
| `sram_compiler/subcircuits/sram_6t_core.py` | `build_array()` 控制等效/真实 cell 实例化 |
| `sram_compiler/subcircuits/sram_10t_core.py` | 10T 版本，等效逻辑与 6T 共用 |
| `sram_compiler/testbenches/parameter_factor.py` | `Sram6TCoreFactory`，将 `use_equivalent` 从 testbench 传到 core |
