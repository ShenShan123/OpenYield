...
Current release: **V2.1.6** (the write drivers take the precharge slot: in a write cycle PRE is inhibited by the held write request and `w_en = we_hold & cs & (wl_en | (pre_ready & pre_off_ready))` turns the drivers on in the clock-high phase, so BL/BLB are at the write rails before the wordline rises; the select is delayed into the precharge gate (`cs_pre`) against the start-up race; the replica write driver writes a 0 with the real ones; checks expect bitlines restored before a read and at the rails before a write (`VWL_WEN_*`, `TWSLOT`, `select_every` idle probes); `timing_lookup.json` is `v2.1.6-timing-5` with every class kept and re-evidenced; `time_generate.py` is rebuilt as builders in signal order with `ControlSizing`, netlist-identical on 1331 netlists; `docs/design/WRITE_SLOT_V2_1_6.md` and `docs/design/TIME_CONTROL_PATH.md`). Driver sizes, the 6T/10T cells and the read path are unchanged. The only remaining item in `docs/plans/V2_1_3_OPEN_ITEMS.md` is the carried Phase 6 scope (extracted metal, half-select write, yield estimator).
