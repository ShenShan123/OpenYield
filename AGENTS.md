...
Current release: **V2.1.8** (no two enables of different roles overlap: a read wordline is released at the sense trigger (`wl_en = WORDLINE_ENABLE_BUFFER(access_clk_bar, s_en_bar)`, a NAND2 first stage), the precharge waits for the previous sense and write enables (`pre_gate = wordline_off & !we_hold & enables_off`, `enables_off = !(s_en | w_en)`), the write slot for the previous sense enable (`write_slot = wordline_off & pre_off_ready & !s_en`), and the write enable ends with the wordline enable rather than the deselect (`w_en = we_hold & (wl_en | (cs_pre & write_slot))`); the column-mux select is a DC level in the testbench; the local checker, validator and scorer search the wordline release from its rise and check the new orderings; `timing_lookup.json` is `v2.1.8-timing-7` with every class kept and re-evidenced; `docs/design/ENABLE_OVERLAP_V2_1_8.md`). V2.1.7 added the rising-edge select gate `cs_pre = cs & cs_delayed` and renamed the block `TIME_CONTROL` (instance `XTIME_CONTROL`, probe paths `XTIME_CONTROL:<node>`, `TimeControlFactory`, `create_time_control_circuit`, inner subcircuits named after their classes, map in `docs/design/TIME_CONTROL_PATH.md` section 6; `docs/design/SELECT_GATE_V2_1_7.md`). V2.1.6 put the write drivers in the precharge slot (`docs/design/WRITE_SLOT_V2_1_6.md`). Driver sizes, the 6T/10T cells and the read path are unchanged. The only remaining item in `docs/plans/V2_1_3_OPEN_ITEMS.md` is the carried Phase 6 scope (extracted metal, half-select write, yield estimator).
