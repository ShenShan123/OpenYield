...
Current release: **V2.1.7** (the control block is `TIME_CONTROL` (instance `XTIME_CONTROL`, probe paths `XTIME_CONTROL:<node>`, `TimeControlFactory`, `create_time_control_circuit`, inner subcircuits named after their classes, map in `docs/design/TIME_CONTROL_PATH.md` section 6); the select of the clock-high enables `cs_pre = cs & cs_delayed` is delayed on its rising edge only and gates both `PRE = NAND3(clk_buf, cs_pre, pre_gate)` and `w_en = we_hold & cs_pre & (wl_en | (pre_ready & pre_off_ready))`, so new write data passes the hold latch before the drivers turn on at an idle -> write edge and an unselected cycle stops both enables at once; a block without the replica guard starts its drivers with `wl_en`; `precharge_guard_stages: 0` builds decks again; every builder honours the passed models; `select_every` write decks change data at the selected edges and the checker tests idle cycles (`strict_idle_*`) and data before the write enable; `timing_lookup.json` is `v2.1.7-timing-6` with every class kept and re-evidenced; `docs/design/SELECT_GATE_V2_1_7.md`). V2.1.6 put the write drivers in the precharge slot (`docs/design/WRITE_SLOT_V2_1_6.md`). Driver sizes, the 6T/10T cells and the read path are unchanged. The only remaining item in `docs/plans/V2_1_3_OPEN_ITEMS.md` is the carried Phase 6 scope (extracted metal, half-select write, yield estimator).
